...
export default function CrosswordJoinLobby() {
  const { code } = useParams<{ code: string }>();
  const navigate = useNavigate();
  const { room, playerId, setPlayerId, emitWithAck } = useCrossword();
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await emitWithAck<{ ok: true; playerId: string }>('crossword:playerJoinRoom', {
        code,
      ...
      saveSession('crossword', code, { playerId: res.playerId });
    } catch (err) {
  ...
  if (!playerId) {
    return (
      <div className="screen screen-center">
        <h1 className="screen-title">Crucigrama {code}</h1>
        <form className="join-form" onSubmit={handleJoin}>
          ...
          <button className="btn btn-primary btn-big" type="submit" disabled={joining}>
            {joining ? 'Entrando...' : '🧩 Entrar al crucigrama'}
          </button>
        </form>
      </div>
    );
